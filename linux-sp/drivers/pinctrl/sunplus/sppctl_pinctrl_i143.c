...
const char *stpctl_m_f_nam(struct pinctrl_dev *_pd, unsigned _fid)
{
...
int stpctl_o_n2map(struct pinctrl_dev *_pd, struct device_node *_dn, struct pinctrl_map **_map, unsigned *_nm)
{
	...
	*_nm = size/sizeof(*list);
	...
	for (i = 0; i < (*_nm); i++) {
		...
		p_p = I143_PCTLD_P(dt_pin);
		p_g = I143_PCTLD_G(dt_pin);
		if ((p_p >= sp7021pins_allSZ)
#ifndef SUPPORT_FULLY_PINMUX
			|| (p_g == I143_PCTL_G_PMUX)
#endif
		) {

...
static int Sd_i143_Bus_Reset_Channel(struct sp_mmc_host *host);
static int Reset_i143_Controller(struct sp_mmc_host *host);
static void sp_i143_mmc_prep_cmd_rsp(struct sp_mmc_host *host, struct mmc_cmd *cmd);
static void sp_i143_mmc_prep_data_info(struct sp_mmc_host *host, struct mmc_cmd *cmd, struct mmc_data *data);
static void sp_i143_mmc_wait_sdstate_new(struct sp_mmc_host *host);
static int sp_i143_mmc_read_data_pio(sp_mmc_host *host,  struct mmc_data *data);
...
void send_i143_stop_cmd(struct sp_mmc_host *host)
{
	struct mmc_cmd stop = {};
	...
	sp_i143_mmc_prep_cmd_rsp(host, &stop);
...
sp_mmc_send_cmd(struct mmc *mmc, struct mmc_cmd *cmd, struct mmc_data *data)
{
	...
	for (i = 0; i < CMD_MAX_RETRY; ++i)
	{
		...
		if (data == NULL) {
			...
			sp_sd_trace();
			sp_i143_mmc_prep_data_info(host, cmd, data);
			sp_mmc_trigger_sdstate(host);
			...
			if(SP_MMC_PIO_MODE == host->dmapio_mode) {
				sp_i143_mmc_read_data_pio(host, data);
			}
...
#ifdef CONFIG_DM_MMC
const struct dm_mmc_ops sp_i143_mmc_ops = {
...
static int sp_143_sd_hw_init(sp_mmc_host *host)
{
...
static int sp_143_sd_hw_set_clock(struct sp_mmc_host *host, uint div)
{
...
static int sp_143_sd_hw_tunel_read_dly (sp_mmc_host *host, sp_mmc_timing_info *dly)
{
...
static int sp_143_sd_hw_tunel_write_dly  (sp_mmc_host *host, sp_mmc_timing_info *dly)
{
...
static int sp_143_sd_hw_tunel_clock_dly  (sp_mmc_host *host, sp_mmc_timing_info *dly)
{
	host->base->sd_clk_dly_sel = dly->clk_dly;
	return 0;
}

int sp_143_sd_hw_highspeed_en (sp_mmc_host *host, bool en)
{
	if (en)
...
int sp_143_sd_hw_set_bus_width (sp_mmc_host *host, uint bus_width)
{
	/* Set the bus width */
	if (bus_width == 4) {
...
int sp_143_sd_hw_set_sdddr_mode (sp_mmc_host *host, int ddrmode)
{
	sp_sd_trace();
	if (ddrmode)
...
int sp_143_sd_hw_set_cmd (sp_mmc_host *host, struct mmc_cmd *cmd)
{
...
int sp_143_sd_hw_get_reseponse (sp_mmc_host *host, struct mmc_cmd *cmd)
{
...
int sp_143_sd_hw_set_data_info (sp_mmc_host *host, struct mmc_cmd *cmd, struct mmc_data *data)
{
	ulong hw_address;
	/* Reset */
	Reset_i143_Controller(host);
	Sd_i143_Bus_Reset_Channel(host);
	/* Configure Group SD Registers */
	host->base->sd_cmdbuf0 = (u8)(cmd->cmdidx | 0x40);	/* add start bit, according to spec, command format */
	host->base->sd_cmdbuf1 = (u8)((cmd->cmdarg >> 24) & 0xff);
	host->base->sd_cmdbuf2 = (u8)((cmd->cmdarg >> 16) & 0xff);
...
int sp_143_sd_hw_trigger (sp_mmc_host *host)
{
	host->base->sdctrl0 = 1;   /* Start transaction */
	return 0;
}

int sp_143_sd_hw_reset_mmc (sp_mmc_host *host)
{
	sp_sd_trace();
...
int sp_143_sd_hw_reset_dma (sp_mmc_host *host)
{
...
int sp_143_sd_hw_reset_sdio (sp_mmc_host *host)
{
	return 0;
}

int sp_143_sd_hw_wait_data_timeout (sp_mmc_host *host, uint timeout)
{
	return 0;
}

int sp_143_sd_hw_check_finish (sp_mmc_host *host)
{
...
int	sp_143_sd_hw_tx_dummy (sp_mmc_host *host)
{
	host->base->sdctrl1 = 1;
	return 0;
}

int sp_143_sd_hw_check_error (sp_mmc_host *host, bool with_data)
{
	int ret = 0;
...
static int sp_143_emmc_hw_init(sp_mmc_host *host)
{
	sp_sd_trace();
...
static int sp_143_emmc_hw_set_clock(struct sp_mmc_host *host, uint div)
{
...
static int sp_143_emmc_hw_tunel_read_dly (sp_mmc_host *host, sp_mmc_timing_info *dly)
{
...
static int sp_143_emmc_hw_tunel_write_dly  (sp_mmc_host *host, sp_mmc_timing_info *dly)
{
...
static int sp_143_emmc_hw_tunel_clock_dly (sp_mmc_host *host, sp_mmc_timing_info *dly)
{
	sp_sd_trace();
...
int sp_143_emmc_hw_highspeed_en (sp_mmc_host *host, bool en)
{
	sp_sd_trace();
	if (en)
...
int sp_143_emmc_hw_set_bus_width (sp_mmc_host *host, uint bus_width)
{
	sp_sd_trace();
	/* Set the bus width */
	if (bus_width == 4) {
...
int sp_143_emmc_hw_set_sdddr_mode (sp_mmc_host *host, int ddrmode)
{
	sp_sd_trace();
	if (ddrmode)
...
int sp_143_emmc_hw_set_cmd (sp_mmc_host *host, struct mmc_cmd *cmd)
{
	sp_sd_trace();
	/* printf("Configuring registers\n"); */
	/* Configure Group SD Registers */
...
int sp_143_emmc_hw_get_reseponse (sp_mmc_host *host, struct mmc_cmd *cmd)
{
	sp_sd_trace();
	unchar *rspBuf = (unchar *)cmd->response;
	int i;
...
int sp_143_emmc_hw_set_data_info (sp_mmc_host *host, struct mmc_cmd *cmd, struct mmc_data *data)
{
	sp_sd_trace();
	ulong hw_address;
	/* Reset */
...
int sp_143_emmc_hw_trigger (sp_mmc_host *host)
{
	sp_sd_trace();
...
int sp_143_emmc_hw_reset_mmc (sp_mmc_host *host)
{
	sp_sd_trace();
...
int sp_143_emmc_hw_reset_dma (sp_mmc_host *host)
{
	sp_sd_trace();
...
int sp_143_emmc_hw_reset_sdio (sp_mmc_host *host)
{
	sp_sd_trace();
	return 0;
}

int sp_143_emmc_hw_wait_data_timeout(sp_mmc_host *host, uint timeout)
{
	sp_sd_trace();
	return 0;
}

int sp_143_emmc_hw_check_finish (sp_mmc_host *host)
{
	sp_sd_trace();
...
int	sp_143_emmc_hw_tx_dummy (sp_mmc_host *host)
{
	sp_sd_trace();
...
int sp_143_emmc_hw_check_error (sp_mmc_host *host, bool with_data)
{
	sp_sd_trace();
	int ret = 0;
...
sp_mmc_hw_ops sd_i143_hw_ops = {
	.hw_init			= sp_143_sd_hw_init,
	.set_clock			= sp_143_sd_hw_set_clock,
	.highspeed_en		= sp_143_sd_hw_highspeed_en,
	.tunel_read_dly		= sp_143_sd_hw_tunel_read_dly,
	.tunel_write_dly	= sp_143_sd_hw_tunel_write_dly,
	.tunel_clock_dly	= sp_143_sd_hw_tunel_clock_dly,
	.set_bus_width		= sp_143_sd_hw_set_bus_width,
	.set_sdddr_mode		= sp_143_sd_hw_set_sdddr_mode,
	.set_cmd			= sp_143_sd_hw_set_cmd,
	.get_response		= sp_143_sd_hw_get_reseponse,
	.set_data_info		= sp_143_sd_hw_set_data_info,
	.trigger			= sp_143_sd_hw_trigger,
	.reset_mmc			= sp_143_sd_hw_reset_mmc,
	.reset_dma			= sp_143_sd_hw_reset_dma,
	.reset_sdio			= sp_143_sd_hw_reset_sdio,
	.wait_data_timeout	= sp_143_sd_hw_wait_data_timeout,
	.check_finish		= sp_143_sd_hw_check_finish,
	.tx_dummy			= sp_143_sd_hw_tx_dummy,
	.check_error		= sp_143_sd_hw_check_error,
};

sp_mmc_hw_ops emmc_i143_hw_ops = {
	.hw_init			= sp_143_emmc_hw_init,
	.set_clock			= sp_143_emmc_hw_set_clock,
	.highspeed_en		= sp_143_emmc_hw_highspeed_en,
	.tunel_read_dly		= sp_143_emmc_hw_tunel_read_dly,
	.tunel_write_dly	= sp_143_emmc_hw_tunel_write_dly,
	.tunel_clock_dly	= sp_143_emmc_hw_tunel_clock_dly,
	.set_bus_width		= sp_143_emmc_hw_set_bus_width,
	.set_sdddr_mode		= sp_143_emmc_hw_set_sdddr_mode,
	.set_cmd			= sp_143_emmc_hw_set_cmd,
	.get_response		= sp_143_emmc_hw_get_reseponse,
	.set_data_info		= sp_143_emmc_hw_set_data_info,
	.trigger			= sp_143_emmc_hw_trigger,
	.reset_mmc			= sp_143_emmc_hw_reset_mmc,
	.reset_dma			= sp_143_emmc_hw_reset_dma,
	.reset_sdio			= sp_143_emmc_hw_reset_sdio,
	.wait_data_timeout	= sp_143_emmc_hw_wait_data_timeout,
	.check_finish		= sp_143_emmc_hw_check_finish,
	.tx_dummy			= sp_143_emmc_hw_tx_dummy,
	.check_error		= sp_143_emmc_hw_check_error,
};
...
int sp_i143_print_mmcinfo(struct mmc *mmc)
{
	struct sp_mmc_host *host = mmc->priv;
...
int sp_i143_mmc_set_dmapio(struct mmc *mmc, uint val)
{
	struct sp_mmc_host *host = mmc->priv;
